...
//, done(false)
//, dusted(false)
, acceptor(io_service, {asio::ip::tcp::v4(), bot.get(PORT_KEY, PORT_VAL)})
{
}
...
//		bug("rserver: accepting:");
//		acceptor.accept(cs);
//		bug("rserver: accepted:");
//
...
//	{
//		asio::ip::tcp::socket cs(io_service);
//		acceptor.async_accept(cs, [&](boost::system::error_code const& ec)
//		{
//			if(ec)
//			{
//				LOG::E << ec.message();
...
void RServerIrcBotPlugin::accept()
{
	auto cs = std::make_shared<asio::ip::tcp::socket>(io_service);

	acceptor.async_accept(*cs,
		std::bind(&RServerIrcBotPlugin::handle_client, this, std::placeholders::_1, cs));
}

void RServerIrcBotPlugin::handle_client(boost::system::error_code const& ec,
	std::shared_ptr<asio::ip::tcp::socket> cs)
{
	...
	if(cs->local_endpoint().address().is_v6())
	{
		auto ip = cs->local_endpoint().address().to_v6();
		if(ipv6accepts.empty() || ipv6accepts.count(ip))
			std::thread(&RServerIrcBotPlugin::process, this, cs).detach();
		else
		{
			cs->close();
			LOG::S << "rejecting unauthorized connection attempt from: " << ip.to_string();
...
void RServerIrcBotPlugin::process(std::shared_ptr<asio::ip::tcp::socket> cs)//asio::ip::tcp::socket cs)
{
	lock_guard lock(mtx);
...
bool RServerIrcBotPlugin::initialize()
{
	...
	accept();
	con = std::async(std::launch::async, [this]{io_service.run();});
